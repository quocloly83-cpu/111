...
const ADMIN_KEY =
  process.env.ADMIN_PASSWORD || "nguyentanhuyvip10thanhngannek";
...
function baseStyles() {
  ...
  <style>
    ...
    *{font-family:"Alata", Arial, sans-serif}
    ...
    .orb.o4{width:180px;height:180px;right:22%;bottom:12%;background:rgba(255,213,107,.16);animation-delay:-3s}
    ...
    .card{
      ...
      border-radius:30px;background:linear-gradient(180deg, rgba(14,10,24,.9), rgba(10,8,19,.84));
      border:1px solid rgba(215,180,255,.16);animation:glow 4.2s infinite;
      backdrop-filter:blur(16px);
      box-shadow:0 0 26px rgba(183,124,255,.15), 0 24px 60px rgba(0,0,0,.35);
      overscroll-behavior:contain;
      position:relative;
      isolation:isolate;
    }
    .card::before{
      content:"";position:absolute;inset:0;padding:1px;border-radius:30px;
      background:linear-gradient(120deg, rgba(255,255,255,.03), rgba(200,107,255,.35), rgba(255,111,216,.22), rgba(255,255,255,.02));
      background-size:200% 200%;animation:borderShift 6s linear infinite;
    ...
    .tile::after{
      ...
      transform:translateX(-120%);opacity:.55;pointer-events:none
    }
    .tile:hover::after{animation:shimmer .9s ease}
...
<body>
  <div class="bgAura"><div class="orb o1"></div><div class="orb o2"></div><div class="orb o3"></div><div class="orb o4"></div></div>
  <div class="wrap">
      ...
      <div class="content">
        ...
        <div id="panelView" class="hidden">
          ...
          <div id="tab1" class="tabPane active">
            ...
            <div class="tile"><div class="row"><div><p class="name">Tâm Bám</p><p class="desc">Tăng độ bám và ổn định kéo ngay sau khi bật</p></div><label class="switch"><input type="checkbox" id="f8" onchange="toggleFx(this,'Tâm Bám')"><span class="slider"></span></label></div></div>
          ...
          <div id="tab5" class="tabPane">
            ...
            <div class="footer">Nút game hỗ trợ Android và iPhone. Bấm vào sẽ ưu tiên mở thẳng ứng dụng trên máy.</div>
          </div>
  ...
  <script>
    ...
    function luuTrangThai() {
      const state = {
        ...
        f8: document.getElementById("f8") ? document.getElementById("f8").checked : false
      };
    ...
    function taiTrangThai() {
      try {
        ...
        ["f1","f2","f3","f4","f5","f6","f7","f8"].forEach(function (id) {
    ...
    function openFF() {
      ...
      if (isAndroid()) {
        openByIntent(FF_ANDROID_PACKAGE);
        return;
      }
      showToast("Thiết bị không hỗ trợ", "err");
    }
    ...
    function openFFMax() {
      ...
      if (isAndroid()) {
        openByIntent(FFMAX_ANDROID_PACKAGE);
        return;
      }
      showToast("Thiết bị không hỗ trợ", "err");
    }
